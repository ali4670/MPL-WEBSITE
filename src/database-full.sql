...
-- Helper functions (must exist before any RLS policy references them)
...
-- Groups (must exist before level_chats references it)
...
-- Ensure missing columns on levels/lectures
...
----------------------------------------------------------------
-- groups table created above; ensure moderator_id exists
...
-- Legacy group_id column on profiles (for backwards compatibility)
...
-- Wrapped in DO blocks because IF EXISTS only handles missing constraints, not missing tables
----------------------------------------------------------------
...
DO $$ BEGIN ALTER TABLE assignment_overrides DROP CONSTRAINT IF EXISTS assignment_overrides_lecture_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE calendar_events DROP CONSTRAINT IF EXISTS calendar_events_lecture_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE calendar_events DROP CONSTRAINT IF EXISTS calendar_events_level_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE student_files DROP CONSTRAINT IF EXISTS student_files_lecture_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE parent_moderator_threads DROP CONSTRAINT IF EXISTS parent_moderator_threads_level_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE grade_history DROP CONSTRAINT IF EXISTS grade_history_lecture_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
DO $$ BEGIN ALTER TABLE grade_history DROP CONSTRAINT IF EXISTS grade_history_exam_id_fkey; EXCEPTION WHEN undefined_table THEN null; END $$;
...
-- 8a. PER-LECTURE CHAT ROOMS (columns already added above)
-- =============================================================

-- =============================================================
-- 8b. GROUP-SCOPED CHAT & TASKS (group_id on level_chats already added above)
-- =============================================================
...
-- groups table and profiles.group_id created above; ensure moderator_id exists
ALTER TABLE groups ADD COLUMN IF NOT EXISTS moderator_id UUID REFERENCES profiles(id) ON DELETE SET NULL;
